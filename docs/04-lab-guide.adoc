...
This section covers the essential components for secure identity and secrets management in Kubernetes.
...
To access the Keycloak web UI, we expose it externally using an Ingress resource instead of port-forwarding.  
This is consistent with the approach used for Vault and better simulates a real-world deployment.

. Deploy the Ingress pointing to the `keycloak` service on port `8080` using the following Helm command:

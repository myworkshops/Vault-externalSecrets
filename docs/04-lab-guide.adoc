...
// - Creating Vault namespaces
// - Installing ESO
// - Syncing secrets per namespace
// - Deploying Keycloak in Dev/QA/Prod
// - Testing sync updates with secrets
...
. Log in using the **Initial Root Token** provided by `vault operator init`.
...
In this section, we install the External Secrets Operator (ESO), which synchronizes secrets from Vault into native Kubernetes `Secret` objects.  
This allows Kubernetes workloads to consume secrets managed in Vault without requiring application-level changes.
...
Each namespace (`dev`, `qa`, `prod`) must define a `SecretStore` resource that configures Vault access using Kubernetes authentication.

To preview the manifest using Helm templating:

[source,bash]
----
helm template vault-store --namespace dev helm/external-secrets/secretStore/ --dry-run=server | yq
----
...
[source,bash]
----
helm upgrade --install vault-store --namespace dev helm/external-secrets/secretStore/
----
...
==== 5. Define an ExternalSecret to sync data from Vault

An `ExternalSecret` defines how ESO should fetch data from Vault and write it into a Kubernetes `Secret`.
...
You should see both `username` and `password` encoded in base64.
...
In this section, we deploy a single instance of Keycloak into the `iam` namespace using the Bitnami Helm chart.  
This Keycloak instance will later host one Realm per environment (`dev`, `qa`, `prod`), acting as the centralized Identity Provider for the platform.
...
We deploy Keycloak with an embedded PostgreSQL database and persistent storage enabled.  
This prevents data loss across pod restarts and ensures the state of realms, users, and configurations is retained.
...
helm upgrade --install keycloak bitnami/keycloak \
  ...
  --set auth.adminPassword=N0TaRandomPassword \
  --set proxy=edge \
  --set service.type=ClusterIP \
  --set externalDatabase.password=N0TaRandomPassword \
  --set postgresql.primary.persistence.enabled=true \
  --set postgresql.primary.persistence.size=1Gi \
  --set postgresql.auth.postgresPassword=N0TaRandomPassword \
  --set postgresql.auth.password=N0TaRandomPassword
----
...
==== 4. Access the Keycloak admin UI (optional)

You can access the Keycloak web UI by port-forwarding:

[source,bash]
----
kubectl port-forward svc/keycloak -n iam 8080:8080
----
...
- A separate Realm for each environment (`dev`, `qa`, `prod`)
- Role-based access control (RBAC), users, clients, and groups
- Integration with Kubernetes and Vault (if needed)

These configurations will be applied manually or automated via CLI/REST/API in a future step.